...
# Use ensemble of states to estimate background error covariance
    ...
    #----
        ...
        rho = 1  # multiplicative inflation
        eigArg = (edim-1)*I/rho + np.dot(C,Yb)
        #eigArg = (edim-1)*I + np.dot(C,Yb)
        #print('eigArg = ', eigArg)
        
        #Pa = np.linalg.inv(eigArg)
    ...
    #----
        ...
        #Wa = np.sqrt((edim-1)) * linalg.sqrtm(Pa)
        #print ('Wa = ')
        #print (Wa)
...
#   Xa = np.dot(Xb,Wa) + xm[:,np.newaxis]
        ...
        #print('Xa = ', Xa)
        '''
        std = np.std(Xa, axis = 1)
        want_dist = 0.5
        
        for p in range(len(std)):
            if std[p] < want_dist:
                Xa_mean = np.mean(Xa[p,:], axis =1)
                Xa_mean = np.matlib.repmat(Xa_mean, 1, edim)
                Xa_dist = Xa[p,:] - Xa_mean
                #print('Xa_dist=',Xa_dist)
                infll = want_dist / std[p]
                print('infll %s = '%(p), infll[0,0])
                Xa_dist_infl = np.dot(Xa_dist, infll[0,0] * np.identity(edim))
                Xa_para = Xa_mean + Xa_dist_infl
                Xa[p,:] = Xa_para
        '''
